...
struct MapParam {
  ...
  // map fusion
  double p_hit_, p_miss_, p_min_, p_max_, p_occ_, p_pred_occ_;  // occupancy probability
  double prob_hit_log_, prob_miss_log_, clamp_min_log_, clamp_max_log_, min_occupancy_log_, pred_occupancy_log_;  // logit
...
struct MapData {
  ...
  std::vector<double> occupancy_buffer_inflate_global_;
...
inline int SDFMap::getInflateOccupancy(const Eigen::Vector3i& id) {
  ...
  double occ = md_->occupancy_buffer_inflate_global_[toAddress(id)];
  if (occ < mp_->clamp_min_log_ - 1e-3) return UNKNOWN;
  if (occ > mp_->min_occupancy_log_) return OCCUPIED;
  return FREE;
}
