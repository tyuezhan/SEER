...
int Astar::search(const Eigen::Vector3d& start_pt, const Eigen::Vector3d& end_pt) {
  ...
  if (edt_env_->sdf_map_->getOccupancy(start_pt) == SDFMap::UNKNOWN) ROS_ERROR("Start unknown");
  if (edt_env_->sdf_map_->getOccupancy(end_pt) == SDFMap::UNKNOWN) ROS_ERROR("Goal unknown");
      ...
      for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
        for (double dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_) {
          ...
          if (edt_env_->sdf_map_->getInflateOccupancy(nbr_pos) == 1 ||
            ...
            // if (edt_env_->sdf_map_->getOccupancy(nbr_pos) == SDFMap::UNKNOWN) ROS_ERROR("A star, nbr unknown");
            continue;
          }
